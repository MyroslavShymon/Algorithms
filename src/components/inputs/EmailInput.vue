<template>
  ...
  >
    ...
    >
      ...
      />
      <!-- :disabled="sending" -->
      <span style="display: none">{{ emailNameOut }}</span>
    </md-field>
    <div :state="valid">
...
export default class EmailInput extends Vue {
  ...
  public get emailNameOut(): boolean {
    this.store.persons.email = this.email;
    this.store.persons.validEmail = this.validEmail;
    return this.store.persons.validEmail;
  }
}
</script>
